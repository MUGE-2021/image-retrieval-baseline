...
def read_submission(submit_path, reference, k=5):
    ...
    with open(submit_path) as fin:
        for line in fin:
            ...
            if "query_id" not in pred_obj:
                raise Exception('There exists one line not containing query_id: {}'.format(line))
            if not isinstance(pred_obj['query_id'], int):
                raise Exception('Found an invalid query_id {}, it should be an integer (not string), please check your schema'.format(qid))
            qid = pred_obj["query_id"]
            ...
            if len(item_ids) != k:
                raise Exception('Query_id {} has wrong number of predicted item_ids! Require {}, but {} founded.'.format(qid, k, len(item_ids)))           
            # check whether there exist an invalid prediction for any query
            for rank, item_id in enumerate(item_ids):
                if not isinstance(item_id, int):
                    raise Exception('Query_id {} has an invalid predicted item_id {} at rank {}, it should be an integer (not string), please check your schema'.format(qid, item_id, rank + 1))
            # check whether there are duplicate predicted products for a single query
            if len(set(item_ids)) != k:
                raise Exception('Query_id {} has duplicate products in your prediction. Pleace check again!'.format(qid))
    ...
    if len(nopred_qids) != 0:
        raise Exception('The following query_ids have no prediction in your submission, please check again: {}'.format(", ".join([str(idx) for idx in nopred_qids])))
...
def read_reference(path):
    ...
    for line in fin:
        line = line.strip()
        obj = json.loads(line)
        reference[obj['query_id']] = obj['item_ids']
    return reference
